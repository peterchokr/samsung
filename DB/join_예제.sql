...
-- 예제 2: 모든 프로젝트와 담당자 (담당자 없는 프로젝트 포함)
SELECT 
    p.project_name AS 프로젝트명,
    p.budget AS 예산,
    COALESCE(e.emp_name, '담당자 미지정') AS 담당자명,
    ep.role AS 역할
...
-- 예제 1: 부서별 직원 수와 평균 급여
SELECT 
    d.dept_name AS 부서명,
    COUNT(e.emp_id) AS 직원수,
    ROUND(AVG(e.salary), 0) AS 평균급여,
    ROUND(SUM(e.salary), 0) AS 총급여
FROM departments d
LEFT JOIN employees e ON d.dept_id = e.dept_id
GROUP BY d.dept_id, d.dept_name
ORDER BY 직원수 DESC;

-- 예제 2: 프로젝트별 참여 인원과 총 작업시간
SELECT 
    p.project_name AS 프로젝트명,
    p.budget AS 예산,
    COUNT(ep.emp_id) AS 참여인원,
    SUM(ep.hours_worked) AS 총작업시간,
    ROUND(p.budget / NULLIF(SUM(ep.hours_worked), 0), 0) AS 시간당비용
FROM projects p
LEFT JOIN employee_projects ep ON p.project_id = ep.project_id
GROUP BY p.project_id, p.project_name, p.budget
ORDER BY 참여인원 DESC;

-- 예제 3: 직원별 참여 프로젝트 수와 총 작업시간
SELECT 
    e.emp_name AS 직원명,
    d.dept_name AS 부서명,
    COUNT(ep.project_id) AS 프로젝트수,
    COALESCE(SUM(ep.hours_worked), 0) AS 총작업시간
FROM employees e
LEFT JOIN departments d ON e.dept_id = d.dept_id
LEFT JOIN employee_projects ep ON e.emp_id = ep.emp_id
GROUP BY e.emp_id, e.emp_name, d.dept_name
ORDER BY 프로젝트수 DESC, 총작업시간 DESC;
...
-- 3. 프로젝트에 참여 중인 모든 직원의 이름, 부서명, 프로젝트명을 조회하세요.
...
-- 4. 프로젝트에 참여하지 않은 직원의 이름과 부서명을 조회하세요.

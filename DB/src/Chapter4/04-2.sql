-- p.114 WHERE 문, = 연산자로 특정 값 조회
SELECT * FROM customer WHERE first_name = 'MARIA';

-- p.116 address_id가 200인 행을 조회
SELECT * FROM customer WHERE address_id = 200;

-- p.117 address_id가 200 미만인 행을 조회
SELECT * FROM customer WHERE address_id < 200;
...
-- p.118 payment_date가 2005-07-09 13:24:07인 행을 조회
...
-- p.119 정해진 범위에 해당하는 데이터 조회
SELECT * FROM customer WHERE address_id BETWEEN 5 AND 10;

-- p.120 2005년 6월 17일~2005년 7월 19일을 포함한 날짜 조회
...
-- p.121 first_name 열에서 M~O 범위의 값을 제외한 데이터 조회
SELECT * FROM customer WHERE first_name NOT BETWEEN 'M' AND 'O';

-- p.122 두 조건을 만족하는 데이터 조회

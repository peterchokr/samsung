-- p.154 첫 번째 열에 AUTO_INCREMENT 적용
...
-- p.155 자동 입력되는 값과 동일한 값을 입력한 경우
...
-- p.158 INSERT INTO와 SELECT로 다른 테이블에 결과 입력
...
-- p.161 부모 테이블에 데이터 입력 후 자식 테이블에도 데이터 입력
...
-- p.162 자식 테이블의 데이터 삭제 후 부모 테이블의 데이터 삭제

-- p.258 ABS 함수에 입력한 숫자를 절댓값으로 반환
...
-- p.259 암시적 형 변환으로 오버플로 없이 절댓값을 반환
SELECT ABS(-2147483648);

-- p.260 SIGN 함수로 입력한 숫자가 양수, 음수, 0인지를 판단
...
-- p.263 LOG 함수로 로그 10을 계산
SELECT LOG(10);

-- p.264 LOG 함수로 로그 10의 5를 계산
SELECT LOG(10, 5);
...
-- p.265 LOG 함수와 EXP 함수로 결과 확인
SELECT EXP(LOG(20)), LOG(EXP(20));

-- POWER 함수로 거듭제곱 계산
SELECT POWER(2,3), POWER(2,10), POWER(2.0, 3);

-- p.266 SQRT 함수로 제곱근 계산
SELECT SQRT(1.00), SQRT(10.00);

-- p.267 RAND 함수로 난수 계산

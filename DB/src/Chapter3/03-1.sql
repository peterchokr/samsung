-- p.79 CREATE 문으로 데이터베이스 생성
CREATE DATABASE doitsql;

-- p.80 DROP 문으로 데이터베이스 삭제
...
-- p.84 DROP 문으로 테이블 삭제
DROP TABLE doit_create_table;

-- p.85 INSERT 문으로 데이터 삽입
...
-- p.93 UPDATE 문으로 테이블 전체 데이터 수정
UPDATE doit_dml SET col_1 = col_1 + 10;

-- p.94 DELETE 문으로 데이터 수정

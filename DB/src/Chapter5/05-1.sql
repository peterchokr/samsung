...
-- p.183 내부 조인한 테이블에서 조건에 맞는 데이터 조회
SELECT
...
-- p.185 2개의 조인 조건으로 조인한 테이블에서 조건에 맞는 데이터 조회
SELECT
...
-- p.187 3개의 테이블을 조인한 테이블에서 조건에 맞는 데이터 조회
SELECT
	a.customer_id, a.first_name, a.last_name,
	b.address_id, b.address, b.district, b.postal_code,
	c.city_id, c.city
...
-- p.190 LEFT OUTER JOIN으로 조회한 결과에서 NULL만 조회
SELECT
	a.address, a.address_id AS a_address_id,
	b.address_id AS b_address_id, b.store_id
FROM address AS a
...
-- p.192 RIGHT OUTER JOIN으로 조회한 결과에서 NULL만 조회
SELECT
...
-- p.194 FULL OUTER JOIN으로 조회한 결과에서 NULL만 조회
SELECT
	a.address_id AS a_address_id, a.store_id,
	b.address, b.address_id AS b_address_id
FROM store AS a
